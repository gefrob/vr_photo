// Added onGazeLongProgress and coolDown
...
var Reticulum = (function () {
    ...
    fuse.initiate = function( options ) {
        ...
        this.coolDown       = parameters.coolDown           ||  1.0;
    ...
    var gazeLong = function( threeObject ) {
        ...
        if( gazeTime >= fuse.duration && !fuse.active  && !fuse.timeDone) {
            ...
            if (threeObject.onGazeLongProgress != null && !fuse.timeDone) {
                threeObject.onGazeLongProgress(gazeTime/fuse.duration);
            }
        }

        if (fuse.timeDone && gazeTime > fuse.coolDown + fuse.duration) {
          fuse.timeDone=false;
          threeObject.userData.hitTime = elapsed;
        }
    };
    ...
    return {
        add: function (threeObject, options) {
            ...
            threeObject.onGazeLongProgress                      = parameters.onGazeLongProgress     || null;
